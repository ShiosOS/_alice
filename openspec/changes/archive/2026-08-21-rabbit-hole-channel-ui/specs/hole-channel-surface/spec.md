...
### Requirement: Warm paper channel visuals
The channel surface and shared product chrome updated for this change MUST use a warm paper background, near-black contemporary grotesque typography (no literary serif as the primary UI type), and a restrained oxide-red accent for primary text actions/links. The system MUST NOT rely on freeform canvas chrome (zoom dock, reset layout) as part of the hole detail experience.

#### Scenario: Channel reads as warm paper product
- **WHEN** the user opens a Rabbit Hole detail page
- **THEN** the channel uses warm paper / grotesque / oxide styling tokens rather than a dark brass freeform map canvas as the primary surface
